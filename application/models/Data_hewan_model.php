...
class Data_hewan_model extends CI_Model{
    ...
    public function getLog($id = null){
        if($id === null){
            ...
            FROM data_hewan DH JOIN jenis_hewan J ON (DH.id_jenis = J.id_jenis) JOIN ukuran_hewan U ON (DH.id_jenis = U.id_ukuran) JOIN member M ON (DH.id_member = M.id_member) JOIN pegawai P1 ON (DH.id_pegawai_cs = P1.id_pegawai) JOIN pegawai P2 ON (DH.id_pegawai_kasir = P2.id_pegawai) JOIN pegawai P3 ON (DH.created_by = P3.id_pegawai) JOIN pegawai P4 ON (DH.deleted_by = P4.id_pegawai)")->result_array();
        } else{
